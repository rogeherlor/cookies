...
p = np.zeros((NN-n_in,3)) # para guardar las posiciones estimadas
v = np.zeros((NN-n_in,3)) # para guardar las velocidades estimadas
r = np.zeros((NN-n_in,3)) # para guardar las orientaciones estimadas
...
for i in range((n_in),NN-1):
   ...
   # estimados por el EKF
    ...
    # CORRECCION DE ESTADO DE NAVEGACION
    ...
    p[i-n_in+1,:] = pIMU.T # posicion
    v[i-n_in+1,:] = vIMU.T # velocidad
    r[i-n_in+1,0] = ypr[2] # orientacion
    r[i-n_in+1,1] = ypr[1]
    r[i-n_in+1,2] = ypr[0]
...
f = pm.geodetic2enu(lla[n_in:,0],lla[n_in:,1],lla[n_in:,2],lla0[0],lla0[1],lla0[2]) # pasar la trayectoria real de geodesicas a ENU

plt.plot(f[0],f[1],'k',label='Trayectoria') # trayectoria real en negro
plt.plot(p[:,0],p[:,1],'b',label='LSTM en EKF') # trayectoria estimada por el sistema en azul

if A!=0 or B!=0: # tramo perdida de datos de GPS
    plt.plot(p[(A-n_in):(B-n_in),0],p[(A-n_in):(B-n_in),1],'r',label='LSTM en EKF en pérdida de GPS') # salida del sistema en rojo
    plt.plot(f[0][(A-n_in):(B-n_in)],f[1][(A-n_in):(B-n_in)],'g',label='Trayectoria en pérdida de GPS') # trayectoria real en verde
plt.legend()
plt.show()
...
vRMS_E = sqrt(mean_squared_error(vel_enu[n_in:,0],v[:,0]))
vRMS_N = sqrt(mean_squared_error(vel_enu[n_in:,1],v[:,1]))
vRMS_U = sqrt(mean_squared_error(vel_enu[n_in:,2],v[:,2]))

#Error orientacion en angulos de Euler
rRMS_R = sqrt(mean_squared_error(orient[n_in:,0],r[:,0]))
rRMS_P = sqrt(mean_squared_error(orient[n_in:,1],r[:,1]))
rRMS_Y = sqrt(mean_squared_error(orient[n_in:,2],r[:,2]))
...
error_E_DKF=np.zeros((p.shape[0],1))
error_N_DKF=np.zeros((p.shape[0],1))

for i in range(0,p.shape[0]-1):
    error_E_DKF[i]= abs(f[0][i]-p[i,0])
    error_N_DKF[i]= abs(f[1][i]-p[i,1])

error_yaw_DKF=np.zeros((1000,1))

# Errores absolutos de orientacion 
for i in range(0,1000):
    error_yaw_DKF[i]= abs(orient[i,2]-r[i,2])

p_DKF = np.zeros((NN-n_in,3))
p_DKF = p
