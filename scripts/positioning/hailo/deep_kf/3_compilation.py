...
Inputs  (next to the script):
    deep_kf_quantized_model.har   — output of 2_optimisation.py

Outputs (next to the script):
    deep_kf.hef                   — on-device executable
    deep_kf_compiled_model.har    — HAR with compilation metadata
    deep_kf_compiled_model.html   — profiler report  (hailo profiler)
    deep_kf_compiled_model.svg    — graph visualisation (hailo visualizer)
"""
...
def init_logging():
    ...
    return logging.getLogger("deep_kf_compilation")
