"""
Tartan IMU HAR → HEF compilation
===================================
...
Inputs  (next to the script):
    tartan_imu_quantized_model.har   — output of 2_optimisation.py

Outputs (next to the script):
    tartan_imu.hef                   — on-device executable
    tartan_imu_compiled_model.har    — HAR with compilation metadata
    tartan_imu_compiled_model.html   — profiler report  (hailo profiler)
    tartan_imu_compiled_model.svg    — graph visualisation (hailo visualizer)
"""
...
def init_logging():
    ...
    return logging.getLogger("tartan_imu_compilation")
