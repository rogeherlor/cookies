...
Inputs  (next to the script):
    tlio_quantized_model.har   — output of 2_optimisation.py

Outputs (next to the script):
    tlio.hef                   — on-device executable
    tlio_compiled_model.har    — HAR with compilation metadata
    tlio_compiled_model.html   — profiler report  (hailo profiler)
    tlio_compiled_model.svg    — graph visualisation (hailo visualizer)
"""
...
def init_logging():
    ...
    return logging.getLogger("tlio_compilation")
