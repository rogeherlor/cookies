...
// WGS84 Constants
static const float a = 6378000.0f;
// static const float f = 1.0f / 298.257223563f;
static const float b = 6357000.0f; // a * (1 - f)
static const float e2 = 0.00657429540102f; // sqrt(1-b^2/a^2)
static const float ep2 = 0.00673949674228f; // e' squared
...
void ENU_to_LLA(float *enu, float lat0, float lon0, float alt0, float *lla) {
    ...
    // 2. ECEF to LLA (Ferraris solution)
    float p2 = x*x + y*y;
    float p = sqrtf(p2);
    float Z2 = z*z;
    float r2 = p2 + Z2;
    ...
    float F = 54.0f * b2 * Z2;
    float G = r2 + (1.0f - e2) * Z2 - e2 * (a2 - b2);
    float c = (e2 * e2 * F * p2) / (G * G * G);
    float s = cbrtf(1.0f + c + sqrtf(c * c + 2.0f * c));
    float k = s + 1.0f + 1.0f / s;
    float P = F / (3.0f * k * k * G * G);
    float Q = sqrtf(1.0f + 2.0f * e2 * e2 * P);
    float r0 = -(P * e2 * p) / (1.0f + Q) + sqrtf((a2 / 2.0f) * (1.0f + 1.0f / Q) - (P * (1.0f - e2) * Z2) / (Q * (1.0f + Q)) - 0.5f * P * p2);
    float U = sqrtf((p - e2 * r0) * (p - e2 * r0) + Z2);
    float V = sqrtf((p - e2 * r0) * (p - e2 * r0) + (1.0f - e2) * Z2);
    float z0_coord = (b2 * z) / (a * V);
    
    lla[2] = U * (1.0f - b2 / (a * V));
    lla[0] = atanf((z + ep2 * z0_coord) / p) * RAD2DEG;
    lla[1] = atan2f(y, x) * RAD2DEG;
}
