...
uint32_t IMU_init(void)
{
  ...
  printf("IMU acc and gyro calibration...");
  int axis = 1; // 0, 1, 2. Originally only 2
  status = ICM20648_accelGyroCalibrate_r(accelBiasScaled, gyroBiasScaled, axis);
...
void IMU_config(float sampleRate)
{
  ...
  BOARD_imuEnableIRQ1(true);
  BOARD_imuEnableIRQ2(true);
  BOARD_imuSetIRQ1Callback(gpioInterrupt);
  BOARD_imuSetIRQ2Callback(gpioInterrupt);
