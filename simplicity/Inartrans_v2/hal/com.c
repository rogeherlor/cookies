...
extern volatile uint8_t gnss_lines_received;  // incremented on each '\n' in USART1 ISR
...
void COM_RX_IRQHandler(COM_Port_t port, uint8_t byte)
{
  ...
  if (byte == UARTDRV_FC_SW_XON) {
    ...
    // store byte in RX FIFO
	  if ((port == comPortUsart1)|(port == COM_USART1)) {									// TODO: buscar aqui
		  //GPIO_PinOutSet(gpioPortA,6);
		  //tengo_gps = gps_inputbyte(USART1->RXDATA);
	  } else {
...
#ifdef COM_USART1_ENABLE
void USART1_RX_IRQHandler(void)				// aqui es donde activamos el flag tengo_gps y mandamos lo que entra por el usart del gps al buffer de entrada
{
  uint32_t flags;
