...
static float Ts1_sum = 0.00001;
static float Ts1_sum_2 = 0.000001;
uint16_t t_max_ciclo=0;			//CUIDADO. Maxima duracion de una iteracion del kalman. Valor hardcodeado. Se utiliza para que el kalman no retrase el tiempo de envio. Se podria eliminar.
...
//		gyroflo[2] = -aux_gyr*PI/180;
		...
		emberAfCorePrint("Ts1 = %ld; Sum = %ld;", (int32_t)(Ts1_sum_2*1000), (int32_t)(Ts1_sum*1000));
		Ts1_sum_2 = 0.000001;
	}
		...
		if(tengo_gps){
			...
			if (validez == 0x41){									//0x41='A' //Feb23 Los mensajes de GNSS guardan los valores antiguos hasta que hay uno nuevo, por lo tanto validez no significa que haya un dato nuevo. Dato nuevo es en busca2()=true
				  ...
				  if(latitud_s==0.0f || longitud_s==0.0f){			//Ene 23. Comprobacion de valor valido. Se puede incluso restringir la zona de operacion a la del pais.
					  ...
					  emberAfCorePrint("\nKUpd; lat_s: %ld, lon_s: %ld, alt_s: %ld; vel_s: %ld; cog_s: %ld", (int32_t)(latitud_s*10000), (int32_t)(longitud_s*10000), (int32_t)(altitud_s*100), (int32_t)(vel_GNSS_u*100), (int32_t)(cog_GNSS_u*100));
					  flag_gps0 = false;
					  cont_invalid_gps = 0;
