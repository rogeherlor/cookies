 ...
 *    Returns zero on OK, non-zero otherwise
 ******************************************************************************/
uint32_t ICM20648_accelGyroCalibrate_r(float *accelBiasScaled, float *gyroBiasScaled, int axis)
{
  uint8_t data[12];
  ...
  // if ( accelBias[2] > 0L ) {
  // x, y, z = 0, 1, 2. Originally only z
  if (axis >= 0 && axis <= 2) {
	  if ( accelBias[axis] > 0L ) {
		accelBias[axis] -= (int32_t) (1.0 / accelRes);
	  } else {
		accelBias[axis] += (int32_t) (1.0 / accelRes);
	  }
  }
